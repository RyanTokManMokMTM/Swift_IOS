...
struct TrackableScrollView<Content>: View where Content: View {
    let axes: Axis.Set //useed to set scrollView
    let showIndicators: Bool //useed to set scrollView
    @Binding var contentOffset: CGFloat //useed to set scrollView offest
    let content: Content
    ...
    init(_ axes: Axis.Set = .vertical, showIndicators: Bool = true, contentOffset: Binding<CGFloat>, @ViewBuilder content: () -> Content) {
        ...
        self.content = content() //pass by user
    }
    ...
    private func calculateContentOffset(fromOutsideProxy outsideProxy: GeometryProxy, insideProxy: GeometryProxy) -> CGFloat {
        if axes == .vertical {
            return outsideProxy.frame(in: .global).minY - insideProxy.frame(in: .global).minY //v = y-y
        } else {
            return outsideProxy.frame(in: .global).minX - insideProxy.frame(in: .global).minX //h =x-x
        }
    }
}
